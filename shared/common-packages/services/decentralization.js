...
export const DecentralizationApi = {
    ...
    async getHistory(data = {}) {
        ...
        data.perFrameIds = data['frameId'].toString();
        delete data.frameId

        return await request(
            'GET',
            `${process.env.API_HOST}auth-command/permission-frame/get-history`,
            data
        )
    },
    async getListFunction(data = {}, useInternal = false, headers = {}) {
        return await request(
          'GET',
          `${Utility.getRemoteHost(useInternal)}auth-query/permission-frame/get-list-functions`,
          {},
          headers
        )
    },
