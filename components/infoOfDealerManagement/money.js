...
function MoneyManagement(props) {
  ...
  useEffect(() => {
    ...
    countMoneyOrder().catch(e => console.log(e));
  }, [])

  const actionButton = (row) => {
    return (
      <More>
        {(row.original.status_id === 1 || row.original.status_id === 10 || row.original.status_id === 5) &&
        <Link href={`${props.isStaff ? ROUTES.ORDER : ROUTES.CRM_ORDER}/${row.original.id}?readOnly`}>
  ...
  const dataTable = () => {
    ...
    const columns = [
      ...
      //   Cell: ({row = {}}) => <span>
      //               <div
      //                 className="font-weight-bold">{row.original?.order_packages?.length > 0 ? row.original?.order_packages[0]?.package?.name || '' : ''}</div>
      ...
      // },
      {
        Header: "Ngày gửi hàng",
        accessor: 'create_at',
        sortable: true,
        className: 'td-6 text-truncate',
        headerClassName: 'td-6 text-truncate',
        Cell: ({row = {}}) => {
          return filters.dateTime(row.original?.order_histories?.find(item => item.title === "Đã lấy")?.updated_at)
        }
      },
      {
        Header: "Ngày giao hàng thành công",
        accessor: 'updated_at',
        sortable: true,
        className: 'td-6 text-truncate',
        headerClassName: 'td-6 text-truncate',
        Cell: ({row = {}}) => {
          return filters.dateTime(row.original?.order_histories?.find(item => item.title === "Giao hàng thành công")?.updated_at)
        }
      },
      // {
      ...
      // },
      {
        ...
        headerClassName: 'td-6 text-truncate',
        sortable: false,
        Cell: ({row = {}}) => [4, 13, 14].includes(row.original.status_id) ?
          <Badge {...statusMappingPayment(row.original.status_id)} isClassicBadge/> : ""
      },
    ];
  ...
  const countMoneyOrder = async () => {
    ...
    try {
      const response = await OrderApi.getCountMoneyOrder(payload);
      if (Response.isSuccessCode(response?.data)) {
        const data = Response.getData(response).data || {};
        if (data) {
          // let _dataCount = [];
  ...
  return (
    <>
      <div className="card-content bg-grey">
        <div className="row">
          <div className="col-3">
            <div className="card-section box-shadow-1 bg-white" style={{height: 80}}>
              <div className="card-body p-1">
                <div className="font-size-xlarge" style={{fontSize: 18}}>
                  Tổng tiền thu hộ <i className='fa fa-info-circle primary' title='Tổng tiền thu hộ dự kiến ở tất cả các trạng thái.'/>
                </div>
                <div className="font-weight-bold teal font-size-large">
                  {dataMoney?.totalCollectionMoney ? filters.currency(dataMoney?.totalCollectionMoney) : 0} đ
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="card-section box-shadow-1 bg-white" style={{height: 80}}>
              <div className="card-body p-1">
                ...
                <div className="font-weight-bold teal font-size-large">
                  {dataMoney?.totalNonControlled ? filters.currency(dataMoney?.totalNonControlled) : 0} đ
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="card-section box-shadow-1 bg-white" style={{height: 80}}>
              <div className="card-body p-1">
                ...
                <div className="font-weight-bold teal font-size-large">
                  {dataMoney?.totalControlled ? filters.currency(dataMoney?.totalControlled) : 0} đ
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="card-section box-shadow-1 bg-white" style={{height: 80}}>
              <div className="card-body p-1">
                ...
                <div className="font-weight-bold teal font-size-large">
                  {dataMoney?.totalPaid ? filters.currency(dataMoney?.totalPaid) : 0} đ
                </div>
              </div>
            </div>
          </div>
      ...
      <ContentWrapper>
        {
          <DataTable {...dataTable()} hasFilter hasSearch={false} filters={[
            {
              label: "Từ ngày",
              type: "datetime",
                             ...
                             <div className="list-group flex-row" id="list-tab"
                               ...
                               <button
                                 ...
                                 className={`btn btn-sm text-center border p-0 list-group-item list-group-item-action
                                      ${statusFilter === 4 && 'btn-teal'}`}
                                 id={`list-dateFilter-${4}`} data-toggle="list"
                                 role="tab"
                                 onClick={() => {
                                   if (statusFilter !== 4) setStatusFilter(4);
                                   else setStatusFilter(null);
                                 }}
                               >
                                 {"Chưa đối soát"} <span
                                 style={{color: statusFilter === 4 ? '' : 'red'}}>{`(${dataCount["totalNonControlled"] || 0})`}</span>
                               </button>
                               ...
                               <button
                                 ...
                                 className={`btn btn-sm text-center border p-0 list-group-item list-group-item-action
                                      ${statusFilter === 13 && 'btn-teal'}`}
                                 id={`list-dateFilter-${13}`} data-toggle="list"
                                 role="tab"
                                 onClick={() => {
                                   if (statusFilter !== 13) setStatusFilter(13);
                                   else setStatusFilter(null);
                                 }}
                               >
                                 {"Đã đối soát"} <span
                                 style={{color: statusFilter === 13 ? '' : 'red'}}>{`(${dataCount["totalControlled"] || 0})`}</span>
                               </button>
                               ...
                               <button
                                 ...
                                 className={`btn btn-sm text-center border p-0 list-group-item list-group-item-action
                                      ${statusFilter === 14 && 'btn-teal'}`}
                                 id={`list-dateFilter-${4}`} data-toggle="list"
                                 role="tab"
                                 onClick={() => {
                                   if (statusFilter !== 14) setStatusFilter(14);
                                   else setStatusFilter(null);
                                 }}
                               >
                                 {"Đã trả tiền"} <span
                                 style={{color: statusFilter === 14 ? '' : 'red'}}>{`(${dataCount["totalPaid"] || 0})`}</span>
                               </button>
                             </div>
                           </li>
                         </ul>
